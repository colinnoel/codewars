...
SELECT y.player_name, y.games, CAST(ROUND((( SUM(y.hits))*1.0 ) / (SUM (y.at_bats)),3) AS varchar) AS batting_average 
FROM yankees AS y                        
GROUP BY y.player_name, games  
HAVING SUM(y.at_bats) >= 100
